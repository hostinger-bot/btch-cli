...
const RELEASE_URL = "https://api.github.com/repos/hostinger-bot/btch-cli/releases/latest";
...
describe("runUpdate", () => {
  it("returns success when the script-managed updater succeeds", async () => {
    vi.doMock("./install-manager", async () => {
      ...
      return {
        ...actual,
        runScriptManagedUpdate: vi.fn().mockResolvedValue({ success: true, output: "Updated to btch 2.0.0." }),
      };
    });

...
import { DEFAULT_MODEL, getEffectiveReasoningEffort, getModelIds, normalizeModelId } from "../provider/models";
...
const USER_DIR = path.join(os.homedir(), ".btch");
...
export function loadProjectSettings(): ProjectSettings {
  const projectPath = path.join(process.cwd(), ".btch", "settings.json");
...
export function getApiKey(): string | undefined {
  return process.env.BTCH_API_KEY || loadUserSettings().apiKey;
}

export function getBaseURL(): string {
  return process.env.BTCH_BASE_URL || "https://ai.tioo.eu.org/v1";
}

export function getCurrentModel(mode?: AgentMode): string {
  if (process.env.BTCH_MODEL) return normalizeModelId(process.env.BTCH_MODEL);
 ...
 * Only BTCH_MODEL env var suppresses this (absolute override). Project-level model
...
export function getModeSpecificModel(mode: AgentMode): string | undefined {
  if (process.env.BTCH_MODEL) return undefined;

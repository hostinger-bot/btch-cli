...
describe("workspace trust settings", () => {
  ...
  it("stores sandbox decisions by canonical workspace path", () => {
    const homeDir = createTempDir("btch-trust-home-");
    const workspace = createTempDir("btch-trust-workspace-");
  ...
  it("preserves existing entries when saving another workspace", () => {
    const homeDir = createTempDir("btch-trust-home-");
    const firstWorkspace = createTempDir("btch-trust-first-");
    const secondWorkspace = createTempDir("btch-trust-second-");
    const trustPath = getWorkspaceTrustPath(homeDir);
  ...
  it("ignores malformed files and entries", () => {
    ...
    expect(loadWorkspaceTrustStore(path.join(homeDir, ".btch", "broken.json"))).toEqual({
      version: 1,
      workspaces: {},
    });
  });

...
const CONNECT_CHANNELS: { id: string; label: string; description: string }[] = [
  { id: "telegram", label: "Telegram", description: "Chat with btch from Telegram" },
];
...
export function App({ agent, startupConfig, initialMessage, onExit }: AppProps) {
  ...
  const [agentsEditorModelIndex, setAgentsEditorModelIndex] = useState(() =>
    Math.max(
      0,
      getAllModels().findIndex((model) => model.id === DEFAULT_MODEL),
    ),
  );
  ...
  // modelListVersion forces a re-read of getAllModels() after a background
  // refresh, so the picker reflects models fetched just now.
  void modelListVersion;
  const allModels = getAllModels();
  const _flatModels = allModels.map((m) => m.id);
  ...
  const openSubagentEditor = useCallback((agent: CustomSubagentConfig | null) => {
    ...
    if (agent) {
      ...
      setAgentsEditorModelIndex(
        Math.max(
          0,
          getAllModels().findIndex((model) => model.id === normalizeModelId(agent.model)),
        ),
      );
    } else {
  ...
  const submitSubagentEditor = useCallback(() => {
    ...
    const model = getAllModels()[agentsEditorModelIndex]?.id;
  ...
  const getTelegramAgent = useCallback(
    (userId: number) => {
      ...
      if (!apiKey) {
        throw new Error("btch API key required. Add it in the CLI or set BTCH_API_KEY.");
      }
  ...
  const submitApiKey = useCallback(() => {
    ...
    if (!apiKey.trim()) {
      setApiKeyError("Enter a valid API key.");
      return;
    }
  ...
  const submitTelegramToken = useCallback(() => {
    ...
    if (!getApiKey()) {
      setTelegramTokenError("Add a btch API key first.");
      return;
    }
    const u = loadUserSettings();
  ...
  const submitTelegramPair = useCallback(async () => {
    ...
    try {
      await bridgeRef.current?.sendDm(result.userId, "Pairing approved. You can message btch here.");
    } catch {
      /* optional DM */
    }
  }, []);
  ...
  const beginTelegramFromConnect = useCallback(() => {
    ...
    if (!getApiKey()) {
      setMessages((p) => [...p, { type: "assistant", content: "Add a btch API key first.", timestamp: new Date() }]);
      openApiKeyModal();
      return;
    }
    if (!getTelegramBotToken()) {
  ...
  const openModelPicker = useCallback(() => {
    ...
    setModelSearchQuery("");
    // Re-fetch the model list from the endpoint so the picker shows the
    // latest models instead of the startup snapshot.
    setModelsRefreshing(true);
    void refreshModels(getBaseURL(), getApiKey()).finally(() => {
      ...
      if (showAgentsEditorRef.current) {
        ...
        if (
          ...
          setAgentsEditorModelIndex((index) =>
            decrement ? Math.max(0, index - 1) : Math.min(getAllModels().length - 1, index + 1),
          );
          return;
        }
        if (key.name === "tab") {
        ...
        >
          ...
          <box flexGrow={1}>
            <textarea
              ...
              placeholder={isProcessing ? "Queue a follow-up... (esc to interrupt)" : placeholder || "Message btch..."}
              textColor={t.text}
              backgroundColor={t.backgroundElement}
    ...
    >
      ...
      >
        ...
        <box flexShrink={0} paddingLeft={2} paddingRight={2} paddingTop={1}>
          <text fg={t.text}>
            {"A new version of btch is available: "}
            <span style={{ fg: t.textMuted }}>
              {"v"}
              {currentVersion}
            </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <box paddingLeft={2} paddingRight={2} paddingTop={1}>
          <text fg={t.text}>
            {"From @BotFather: /newbot, then paste the token here. Stored in ~/.btch/user-settings.json."}
          </text>
        </box>
        <box paddingLeft={2} paddingRight={2} paddingTop={1}>
          <box backgroundColor={t.backgroundElement} paddingLeft={1} paddingRight={1} width="100%">
    ...
    >
      ...
      >
        <box flexShrink={0} flexDirection="row" justifyContent="space-between" paddingLeft={2} paddingRight={2}>
          ...
          <text fg={refreshing ? t.accent : t.textMuted}>{refreshing ? "refreshing..." : "esc"}</text>
        </box>
        <box flexShrink={0} paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
          <text fg={t.text}>{searchQuery || <span style={{ fg: t.textMuted }}>{"Search..."}</span>}</text>
        </box>

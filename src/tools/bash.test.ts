...
describe("shouldRunOnHostInSandboxMode", () => {
  ...
  it("allows compound commands with only safe prefixes like mkdir and sleep", () => {
    expect(
      shouldRunOnHostInSandboxMode(
        "mkdir -p .btch/verify-artifacts && agent-browser --session verify open http://127.0.0.1:3000",
    ...
    expect(
      shouldRunOnHostInSandboxMode(
        "mkdir -p .btch/verify-artifacts && agent-browser open http://127.0.0.1:3000 && agent-browser screenshot",
        { hostBrowserCommandsOnHost: true },
      ),
    ).toBe(true);
  });
...
describe("wrapHostBrowserCommand", () => {
  it("injects a deterministic CLI fallback wrapper", () => {
    ...
    expect(result).toContain("__btch_ab()");
    ...
    expect(result).toContain("__btch_ab open http://127.0.0.1:3000");
    expect(result).toContain("__btch_ab screenshot out.png");
  });

  it("handles session flags and screenshot paths correctly", () => {
    const result = wrapHostBrowserCommand(
      "agent-browser --session verify screenshot .btch/verify-artifacts/verify-smoke-home.png",
    );
    expect(result).toContain("__btch_ab --session verify screenshot .btch/verify-artifacts/verify-smoke-home.png");
  });
});
...
describe("BashTool sandbox state", () => {
  it("tracks cwd changes independently of sandbox mode", () => {
    const root = makeTempDir("btch-bash-test-");

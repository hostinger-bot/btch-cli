...
export class BashTool {
  ...
  private async ensureTmpDir(): Promise<string> {
    if (!this.tmpDir) {
      this.tmpDir = await mkdtemp(path.join(os.tmpdir(), "btch-bg-"));
    }
    return this.tmpDir;
  }
...
export function wrapHostBrowserCommand(command: string): string {
  const normalized = command
    .replace(/\bbunx\s+agent-browser\b/g, "__btch_ab")
    .replace(/\bnpx(?:\s+-y)?\s+agent-browser\b/g, "__btch_ab")
    .replace(/\bagent-browser\b/g, "__btch_ab");
  return [
    "__btch_ab() {",

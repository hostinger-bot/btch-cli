...
const SCHEDULES_DIR = path.join(os.homedir(), ".btch", "schedules");
const SCHEDULE_DAEMON_PID_PATH = path.join(os.homedir(), ".btch", "daemon.pid");
...
export async function startScheduleDaemon(cwd = process.cwd()): Promise<ScheduleDaemonStartResult> {
  ...
  const child = spawn(process.execPath, [...resolveCliArgs(), "daemon"], {
    ...
    detached: true,
    stdio: "ignore",
    env: { ...process.env, FORCE_COLOR: "0", BTCH_DAEMON_CHILD: "1" },
  });
  child.unref();

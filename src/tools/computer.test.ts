...
describe("computer tools", () => {
  ...
  it("stores screenshots under .btch/computer by default", () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "btch-computer-"));
    tempDirs.push(cwd);

    const outputPath = buildScreenshotPath(cwd);

    expect(outputPath).toContain(path.join(".btch", "computer"));
    expect(path.dirname(outputPath)).toBe(path.join(cwd, ".btch", "computer"));
  });
  ...
  it("types into an element ref", async () => {
    ...
    const result = await computerType({ ref: "@e5", text: "hello from btch" }, "/tmp", undefined, runner);

    expect(result.success).toBe(true);
    expect(result.output).toContain("Typed into @e5.");
    expect(runner).toHaveBeenCalledWith(["type", "@e5", "hello from btch"], "/tmp", undefined);
  });

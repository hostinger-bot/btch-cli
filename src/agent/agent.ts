...
import { DEFAULT_MODEL, getModelInfo, normalizeModelId } from "../provider/models";
import { createTools } from "../provider/tools";
import {
  appendCompaction,
  appendMessages,
  appendSystemMessage,
...
const MODE_PROMPTS: Record<AgentMode, string> = {
  agent: `You are btch CLI in Agent mode — a powerful AI coding agent. You execute tasks directly using tools.
...
Be direct. Execute, don't just describe. Show results, not plans.`,
  ...
  plan: `You are btch CLI in Plan mode — you analyze and plan but DO NOT execute changes.
...
- NEVER create, modify, or delete files — only read and analyze`,
  ...
  ask: `You are btch CLI in Ask mode — you answer questions clearly and thoroughly.
...
function formatCustomSubagentsPromptSection(subagents: CustomSubagentConfig[]): string {
  ...
  return `\n\nCUSTOM SUB-AGENTS:\nUser-defined foreground sub-agents from ~/.btch/user-settings.json. When one matches the task, call the task tool with agent set to the exact name.\n\n${lines.join("\n\n")}\n`;
}
...
function buildSubagentPrompt(
    ...
    : request.agent === "explore"
      ...
      : isVision
        ...
        : isVerifyDetect
          ...
          : isVerifyManifest
            ? "You are the Verify Manifest sub-agent. You inspect a repository and create or update .btch/environment.json so verification can run reproducibly."
            : isVerify
  ...
  const rules = isExplore
    ...
    : isVerifyDetect
      ...
      : isVerifyManifest
        ? [
            "Focus on creating or updating .btch/environment.json as the primary verification contract for this repository.",
            "Read package.json and key config files to understand the project, then write .btch/environment.json.",
            "Prefer editing only .btch/environment.json unless the delegated task explicitly requires something else.",
            "",
...
export class Agent {
  ...
  constructor(
    ...
    const envMax = Number(process.env.BTCH_MAX_TOKENS);
  ...
  async runTaskRequest(
    ...
    if (
      ...
      const message = `Unknown sub-agent "${agentKey}". Use general, explore, vision, verify, verify-detect, verify-manifest, computer, or a configured name from ~/.btch/user-settings.json.`;
      return {
        success: false,
        output: message,
        task: {
    ...
    const childBaseTools = createTools(childBash, childMode);
    ...
    try {
      ...
      const childPrompt =
        isVerify && verifyPreparedRecipe
          ? `${request.prompt}\n\nPrepared verify recipe JSON (use this as the primary execution recipe and keep .btch/environment.json aligned with it if present):\n${JSON.stringify(verifyPreparedRecipe, null, 2)}`
          : request.prompt;
    ...
    try {
      while (true) {
        ...
        try {
          ...
          const baseTools = createTools(this.bash, this.mode, {

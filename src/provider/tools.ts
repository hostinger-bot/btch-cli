...
export function createTools(bash: BashTool, mode: AgentMode = "agent", options: CreateToolsOptions = {}) {
  ...
  if (mode === "agent") {
    ...
    tools.computer_screenshot = tool({
      ...
      inputSchema: z.object({
        output_path: z
          .string()
          .optional()
          .describe("Optional output path for the screenshot. Defaults to .btch/computer/*.png"),
    ...
    if (options.scheduleManager) {
      ...
      tools.schedule_create = tool({
        description:
          "Create a recurring or one-time scheduled headless run. Provide a name, the instruction to run, and a cron expression for recurring schedules. Omit cron for an immediate one-time run.",
        inputSchema: z.object({
          name: z.string().describe("Human-readable schedule name"),
          instruction: z.string().describe("The prompt/instruction the agent should run headlessly"),
    ...
    execute: async () => {
      try {
        ...
        if (!WalletManager.exists()) {
          return { success: false, output: "No wallet found. Run `btch wallet init` to create one." };
        }
...
function formatScheduleList(schedules: StoredSchedule[], daemonStatus: ScheduleDaemonStatus): string {
  ...
  if (!daemonStatus.running) {
    lines.push("");
    lines.push("Start `btch daemon` to run recurring schedules.");
  }
...
function formatDaemonReminder(status: ScheduleDaemonStatus): string {
  ...
  return "Daemon status: not running. Use `schedule_daemon_start` (or `btch daemon`) to run recurring schedules.";
}

...
import { DEFAULT_MODEL, getAllModels, normalizeModelId, refreshModels } from "./provider/models";
...
async function startInteractive(
  ...
  // Disable OpenTUI's Kitty graphics protocol detection unless the user opted in.
  // The detection query (`\x1b_Gq…`) leaks raw responses like `Gi=31337,s=1,v=1,a=q,t=d,f=24;AAAA`
  // into the pane when running inside tmux (upstream opentui issue #334). The TUI does
  // not render terminal images, so graphics detection is unnecessary here.
...
async function promptWorkspaceTrust(
  ...
  const message = sandboxSupported
    ? [
        "",
        `btch has not been run in ${cwd} before.`,
        "",
        ...
        "",
        "Run btch in sandbox mode for this directory?",
        "",
    ...
    : [
        ...
        "",
        "Run btch directly on the host for this directory?",
        "",
...
async function resolveWorkspaceTrustSandboxMode(sandboxMode: SandboxMode, options: CliOptions): Promise<SandboxMode> {
  ...
  if (process.env.BTCH_TRUST_WORKSPACE) return sandboxMode;
...
async function runBackgroundDelegation(jobPath: string, options: CliOptions) {
  ...
  try {
    ...
    if (!apiKey) {
      throw new Error("API key required. Set BTCH_API_KEY, use --api-key, or save it to ~/.btch/user-settings.json.");
    }
...
function requireApiKey(apiKey: string | undefined): string {
  if (!apiKey) {
    console.error(
      "Error: API key required. Set BTCH_API_KEY env var, use --api-key, or save to ~/.btch/user-settings.json",
    );
    process.exit(1);
  }
...
program
  ...
  .option("-k, --api-key <key>", "btch API key")
  ...
  .option("--update", "Update btch to the latest version and exit")
  .action(async (message: string[], options) => {
    ...
    if (config.apiKey) {
      await refreshModels(config.baseURL, config.apiKey);
    }
...
program
  ...
  .action(async () => {
    ...
    if (apiKey) {
      await refreshModels(getBaseURL(), apiKey);
    }
    console.log("\nAvailable Models:\n");
    for (const m of getAllModels()) {
      const tags = [m.reasoning ? "reasoning" : "non-reasoning"].filter(Boolean);
...
program
  .command("update")
  .description("Update btch to the latest release")
  .action(async () => {
    console.log("Checking for updates...");
    const result = await runUpdate(packageJson.version);
    console.log(result.output);
...
program
  .command("uninstall")
  .description("Remove a script-installed btch binary and optional data")
  .option("--dry-run", "Show what would be removed without removing it")
  .option("--force", "Skip the confirmation prompt")
  .option("--keep-config", "Keep ~/.btch config files")
  .option("--keep-data", "Keep ~/.btch data files")
  .action(async (options) => {
...
walletCommand
  ...
  .action(async (options) => {
    ...
    console.log("\nPayments have been enabled in ~/.btch/user-settings.json.");
  });

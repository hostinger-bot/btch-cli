...
export function buildVerifyDetectPrompt(cwd: string, settings?: SandboxSettings): string {
  ...
  return [
    ...
    "- If environment.json or .btch/environment.json exists, treat it as the highest-priority source of truth and only fill in missing details.",
...
export function buildVerifyPrompt(cwd: string): string {
  ...
  return [
    ...
    "",
    "Step 1: Run the `verify-manifest` sub-agent to create or update `.btch/environment.json`.",
    ...
    "```",
    "",
    "- Tell the sub-agent: Review this draft recipe against package.json and key config files. Adjust if needed, then write .btch/environment.json. If the draft looks correct, write it as-is. Do not over-research.",
    ...
    "- Tell it to use `.btch/environment.json` as the source of truth.",
    ...
    "  - Use `agent-browser record start .btch/verify-artifacts/verify-smoke.webm` before opening the page.",
    "  - Use `agent-browser --screenshot-dir .btch/verify-artifacts screenshot` after the page loads.",

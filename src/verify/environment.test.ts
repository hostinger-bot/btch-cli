...
describe("verify environment manifest", () => {
  it("loads a root environment.json manifest and normalizes sandbox settings", () => {
    const dir = makeTempDir("btch-verify-env-");
    fs.writeFileSync(
  ...
  it("prefers .btch/environment.json over the root manifest", () => {
    const dir = makeTempDir("btch-verify-env-precedence-");
    fs.writeFileSync(
      path.join(dir, "environment.json"),
      JSON.stringify({
        ecosystem: "node",
    ...
    fs.mkdirSync(path.join(dir, ".btch"));
    fs.writeFileSync(
      path.join(dir, ".btch", "environment.json"),
      JSON.stringify({
        ecosystem: "node",
        appKind: "nextjs",
        appLabel: "Nested",
    ...
    expect(loaded?.path).toBe(path.join(dir, ".btch", "environment.json"));
    expect(loaded?.recipe.appKind).toBe("nextjs");
    expect(loaded?.recipe.installCommands).toEqual(["npm ci"]);
  });

  it("writes a generated .btch/environment.json manifest", () => {
    const dir = makeTempDir("btch-verify-env-write-");
    ...
    expect(written).toBe(path.join(dir, ".btch", "environment.json"));
    const loaded = loadVerifyEnvironment(dir);

...
describe("verify orchestrator", () => {
  it("uses the environment manifest as the highest-priority recipe source", async () => {
    const dir = makeTempDir("btch-verify-orch-manifest-");
    fs.writeFileSync(
  ...
  it("creates .btch/environment.json from verify-detect when no manifest exists", async () => {
    const dir = makeTempDir("btch-verify-orch-generate-");
    fs.writeFileSync(
    ...
    const prepared = await prepareVerifyRun(agent, {});
    expect(prepared.manifestPath).toBe(path.join(dir, ".btch", "environment.json"));
  ...
  it("does not create a manifest when verify-detect returns no recipe", async () => {
    const dir = makeTempDir("btch-verify-orch-no-manifest-");
    fs.writeFileSync(
      path.join(dir, "package.json"),
      JSON.stringify({ dependencies: { next: "15.0.0" }, scripts: { dev: "next dev", build: "next build" } }, null, 2),
    );
    ...
    expect(fs.existsSync(path.join(dir, ".btch", "environment.json"))).toBe(false);
    expect(prepared.usedVerifyDetect).toBe(false);
  });

  it("restores sandbox settings after running verification and wires checkpoint settings", async () => {
    const dir = makeTempDir("btch-verify-orch-run-");
    fs.writeFileSync(
      path.join(dir, "package.json"),
      JSON.stringify({ dependencies: { next: "15.0.0" }, scripts: { dev: "next dev", build: "next build" } }, null, 2),
    );
    ...
    ensureVerifyCheckpointMock.mockResolvedValue({
      ...
      guestWorkdir: "/btch/verify/worktree",
    });

...
export const VERIFY_ARTIFACT_DIR = ".btch/verify-artifacts";
...
export function buildReadinessGuidance(profile: VerifyProjectProfile): string[] {
  if (profile.recipe.smokeKind !== "http" || !profile.recipe.startCommand || !profile.recipe.startPort) {
    return [
      "- If you start any process, capture stdout/stderr into a log file under .btch/verify-artifacts whenever practical.",
    ];
  }
...
export function buildBrowserGuidance(profile: VerifyProjectProfile): string[] {
  if (profile.recipe.smokeKind === "http" && profile.recipe.smokeTarget) {
    return [
      ...
      "- IMPORTANT: All artifact paths MUST be absolute. Use `$(pwd)/${VERIFY_ARTIFACT_DIR}` to construct them. Example: `$(pwd)/.btch/verify-artifacts/verify-smoke.webm`.",
      "",

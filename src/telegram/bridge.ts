...
export function createTelegramBridge(opts: TelegramBridgeOptions): TelegramBridgeHandle {
  ...
  const ensureApprovedUser = async (ctx: { from?: { id?: number }; reply: (text: string) => Promise<unknown> }) => {
    ...
    if (!approved.includes(userId)) {
      await ctx.reply("Not paired yet. Send /pair to get a code, then approve in btch CLI.");
      return null;
    }
  ...
  bot.command("start", async (ctx) => {
    await ctx.reply("Send /pair to link this chat to btch CLI, then approve the code in the terminal.");
  });
  ...
  bot.command("pair", async (ctx) => {
    ...
    await ctx.reply(`Your pairing code: ${code}\nEnter this code in btch CLI (/remote-control → Telegram) to approve.`);
  });

...
agent-browser record start .btch/verify-artifacts/verify-smoke.webm

# Capture screenshot proof (use --screenshot-dir, not a positional path)
mkdir -p .btch/verify-artifacts
agent-browser --screenshot-dir .btch/verify-artifacts screenshot
agent-browser --screenshot-dir .btch/verify-artifacts screenshot --full
...
- Save screenshots to stable workspace-relative paths under `.btch/verify-artifacts/` so verify can report them back to the user.

...
### Changed
- Rebranded to **btch-cli** — no Grok/xAI branding; binaries and data dirs renamed (`~/.btch`, `.btch/`, `BTCH_*` env vars)
- Switched provider to any OpenAI-compatible endpoint via `@ai-sdk/openai-compatible` (default `https://ai.tioo.eu.org/v1`)
- Model list is now auto-fetched from the endpoint's `/v1/models` on startup
- Removed xAI-only features: X/web search, image & video generation, Batch API, Grok STT (voice notes now fall back to no transcription)

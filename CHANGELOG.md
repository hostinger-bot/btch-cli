...
### Fixed
- Termux: `btch uninstall` now runs in the Termux wrapper (before entering proot), so it removes the binary, wrapper, and `~/.btch` correctly instead of reporting "not script-managed"

## [3.0.13] - 2026-08-18

### Fixed
- Termux installer: keep the CLI binary in Termux home (`~/.btch/bin/btch`) and bind-mount it into Debian at runtime instead of writing into the proot rootfs (whose location varies) — `btch` now works reliably from Termux
